...
def select_data_file():
    data_path.set(filedialog.askopenfilename(title="YAML файл данных"))

def select_model_file():
    # model_path.set(filedialog.askopenfilename(title="Файл весов модели"))
    model_path.set("yolo_actual1.pt") # или попробуй yolo_actual2.pt

def select_source_file():
    source_path.set(filedialog.askopenfilename(title="Файл для трекинга"))
    # source_path.set("test3.mp4")
...
app = tk.Tk()
app.title("YOLO Training and Tracking Interface")
...
Label(controls_frame, text="Файл для трекинг:", font=FONT, bg=LABEL_BG, fg=LABEL_FG).grid(row=2, column=0, pady=5, sticky="w")
...
ttk.Button(controls_frame, text="Запустить трекинг", command=start_prediction, style="ElegantButton.TButton").grid(row=5, column=0, pady=10, sticky="w")
